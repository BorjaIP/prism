...
class TestBuildPrompt:
    def test_includes_pr_title(self) -> None:
        prompt = AIService._build_prompt(_make_pr(title="My PR"), _make_file())
        assert "My PR" in prompt

    def test_includes_pr_author(self) -> None:
        prompt = AIService._build_prompt(_make_pr(author="bob"), _make_file())
        assert "bob" in prompt

    def test_includes_filename(self) -> None:
        prompt = AIService._build_prompt(_make_pr(), _make_file(filename="core/utils.py"))
        assert "core/utils.py" in prompt

    def test_includes_file_status(self) -> None:
        prompt = AIService._build_prompt(_make_pr(), _make_file(status="modified"))
        assert "modified" in prompt

    def test_includes_additions_and_deletions(self) -> None:
        prompt = AIService._build_prompt(_make_pr(), _make_file(additions=10, deletions=3))
        assert "+10" in prompt
        assert "-3" in prompt

    def test_includes_diff_patch(self) -> None:
        prompt = AIService._build_prompt(_make_pr(), _make_file(patch="+ new line\n"))
        assert "new line" in prompt

    def test_truncates_patch_to_8000_chars(self) -> None:
        long_patch = "+" + "x" * 9000
        prompt = AIService._build_prompt(_make_pr(), _make_file(patch=long_patch))
        assert len(prompt) < 9000 + 500

    def test_truncates_body_to_2000_chars(self) -> None:
        long_body = "Z" * 3000
        prompt = AIService._build_prompt(_make_pr(body=long_body), _make_file())
        assert prompt.count("Z") == 2000

    def test_uses_none_placeholder_for_empty_body(self) -> None:
        prompt = AIService._build_prompt(_make_pr(body=""), _make_file())
        assert "(none)" in prompt

    def test_includes_pr_description(self) -> None:
        prompt = AIService._build_prompt(_make_pr(body="Fixes the memory leak"), _make_file())
...
class TestParseResponse:
    ...
    def test_parses_valid_json(self) -> None:
        result = AIService._parse_response(self._valid_json())
    ...
    def test_strips_markdown_fences(self) -> None:
        raw = "```json\n" + self._valid_json() + "\n```"
        result = AIService._parse_response(raw)
        assert isinstance(result, AIAnalysis)
        assert result.risk == "low"
    ...
    def test_raises_when_api_key_missing(self) -> None:
        ...
        with patch.dict(os.environ, env, clear=True):
            with pytest.raises(RuntimeError, match="ANTHROPIC_API_KEY"):
                AIService._get_anthropic_client()
...
class TestAnalyzeFileSkipsRemoved:
    def test_returns_static_analysis_for_removed_file(self, tmp_path) -> None:
        pr = _make_pr()
        removed_file = _make_file(status="removed")

        result = AIService(cache_dir=tmp_path / "cache").analyze_file(pr, removed_file, "o/r", 1)
